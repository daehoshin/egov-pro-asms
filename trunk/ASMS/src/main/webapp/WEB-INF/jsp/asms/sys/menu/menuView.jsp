	...
	<div class="modal-content">
        ...
        <div class="modal-footer">
        	<div class="pull-left">
        		<c:choose>
					<c:when test="${loginUserVO.user_auth_cd eq 1 }">
        				<button type="button" class="btn btn-warning" onclick="jsMenuRegFinishAction('${placeVO.plc_id }','${placeVO.reg_status }')">등록완료</button>
        				<button type="button" class="btn btn-danger" onclick="jsMenuDel('${placeVO.plc_id }','${placeVO.reg_status }')">삭제</button>
            		</c:when>
            		<c:otherwise>
		            	<button type="button" class="btn btn-warning" disabled="disabled" onclick="jsMenuRegFinishAction('${placeVO.plc_id }')">등록완료</button>
		            	<button type="button" class="btn btn-danger" disabled="disabled" onclick="jsMenuDel('${placeVO.plc_id }')">삭제</button>
            		</c:otherwise>
            	</c:choose>
        	</div>
        	<div class="pull-right">
        		<c:choose>
        			<c:when test="${loginUserVO.user_auth_cd eq 1 }">
		        		<button type="button" class="btn btn-primary" onclick="jsMenuModForm('${placeVO.plc_id }')">수정</button>
		            </c:when>
		            <c:otherwise>
		            	<button type="button" class="btn btn-primary" disabled="disabled" onclick="jsMenuModForm('${placeVO.plc_id }')">수정</button>
		            </c:otherwise>
		        </c:choose>
